...
function getInfo(button) {
  ...
  authService
    .fetchWithToken(`/med-center/api/patients/${patientId}/medRecords`)
    .then((response) => {
      if (!response.ok) {
    ...
    .then((data) => {
      ...
      medRecordsTable.className =
        "table table-bordered table-condensed table-striped";
      medRecordsTable.innerHTML = `<thead><tr><th  colspan=5>${patientName}'s MedRecords</th><td colspan=2><button class='btn btn-info btn-sm ms-2' onclick='showAddMedRecordModalW(this)'>Add medRecord</button></td></tr>`;
      medRecordsTable.innerHTML +=
        "<tr><th>ID</th><th>Diagnosis</th><th>Date</th><th>Doctor</th><th>Medications</th><th>Action</th><th>Action</th></tr></thead><tbody></tbody>";
...
function deleteApp(button) {
  ...
  authService
    .fetchWithToken(`/med-center/api/patients/medRecords/${appId}`, {
      method: "DELETE",
      headers: {
...
function updateMedRecord(event) {
  ...
  var medRecordIdPatient = document.getElementById(
    "updateMedRecordIdPatient",
  ).value;
  var medRecordIdDoctor = document.getElementById(
    "updateMedRecordIdDoctor",
  ).value;
  ...
  authService
    .fetchWithToken("/med-center/api/patients/medRecords", {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },
...
function showUpdateMedRecordModalW(button) {
  ...
  var updateMedRecordModal = new bootstrap.Modal(
    document.getElementById("updateMedRecordModal"),
  );
...
function showAddMedRecordModalW(event) {
  var addMedRecordModal = new bootstrap.Modal(
    document.getElementById("addMedRecordModal"),
  );
...
function fillDoctorCombo(combo, callback) {
  authService
    .fetchWithToken("/med-center/api/doctors")
    .then((response) => {
      if (!response.ok) {
        throw new Error("Network response was not ok");
      }
...
function createMedRecordRow(medRecord) {
  ...
  medRecordRow.innerHTML =
    ...
    "</td>" +
    "<td>" +
    getMedicationsString(medRecord) +
    "</td>" +
